...
function syncNotifications(ns){const unread=(Array.isArray(ns)?ns:[]).filter(n=>!n.read).map(cleanItem).slice(0,20);setState({suggestion:unread.some(n=>n.type==='suggestion'),other:unread.some(n=>n.type!=='suggestion'),items:unread})}
...
async function refresh(force=false){const id=userId(),now=Date.now();if(!id||busy||(!force&&now-lastRefresh<MIN_REFRESH_GAP))return false;busy=true;lastRefresh=now;try{const j=await fetchJson(API+'?action=listNotifications&userId='+encodeURIComponent(id)+'&t='+now);if(j?.ok&&Array.isArray(j.notifications)){syncNotifications(j.notifications);return true}}catch(e){console.warn('Badges notification',e)}finally{busy=false}return false}
...
function markItemRead(notificationId){const id=String(notificationId||'');const item=state.items.find(x=>String(x.id)===id);state.items=state.items.filter(x=>String(x.id)!==id);recalc();if(item?.id&&!item.id.startsWith('local-'))markServerRead(item.id).finally(()=>setTimeout(()=>refresh(true),500));return true}
function markSuggestionRead(suggestionId=''){const key=String(suggestionId||'');const matches=state.items.filter(x=>x.type==='suggestion'&&(!key||String(x.data?.suggestionId||x.data?.row||'')===key));state.items=state.items.filter(x=>!(x.type==='suggestion'&&(!key||String(x.data?.suggestionId||x.data?.row||'')===key)));state.suggestion=state.items.some(x=>x.type==='suggestion');if(!matches.length)state.suggestion=false;save();render();Promise.all(matches.filter(n=>n.id&&!n.id.startsWith('local-')).map(n=>markServerRead(n.id))).finally(()=>setTimeout(()=>refresh(true),500));return true}
function clearAll(){setState({suggestion:false,other:false,items:[]});markAllServerRead().finally(()=>setTimeout(()=>refresh(true),500));return true}
function pushType(e){const d=e?.detail||{},data=d.data||d.additionalData||{};return String(data.type||d.type||'').toLowerCase()}
function onPush(e){const d=e?.detail||{},type=pushType(e)||'info';addItem({id:d.id||'',type,title:d.title||'Notification',message:d.message||d.body||'',createdAt:new Date().toISOString(),data:d.data||d.additionalData||{}});setTimeout(()=>refresh(true),1000)}
...
window.addEventListener('pageshow',()=>{load();render();refresh(true)});
...
window.HorticultureLightBadges={refresh:()=>refresh(false),refreshNow:()=>refresh(true),render,getItems:fallbackItems,getState:()=>({...state,items:state.items.slice()}),markItemRead,markSuggestionRead,clearAll};
})();