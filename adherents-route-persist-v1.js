...
function openAdherents(){if(restoring)return;const fn=window.HorticultureAdherents?.open;if(typeof fn!=='function')return;restoring=true;setView(VIEW);try{fn()}finally{setTimeout(()=>{restoring=false},350)}}
...
document.addEventListener('click',e=>{const b=e.target.closest?.('button,[data-permission],[data-go],.nav');if(!b)return;if(isAdherentsTrigger(b)){setView(VIEW);return}if(b.closest?.('#adherentsAdmin')&&b.matches?.('[data-close],.aa-back')){setView('');deactivateAdherents();return}if(b.dataset?.go||b.classList?.contains('nav')){setView('');deactivateAdherents()}},true);
