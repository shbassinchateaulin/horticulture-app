(()=>{
'use strict';
if(window.__horticultureUnifiedModuleIconsV1)return;window.__horticultureUnifiedModuleIconsV1=true;
...
function h(btn){return btn?.querySelector?.('.ico,.dashIcon,.spaceIcon')||null}
function apply(){
  document.querySelectorAll('[data-module="messaging"]').forEach(btn=>{const x=h(btn);if(!x)return;x.style.backgroundImage='none';x.style.padding='0';x.innerHTML=CHAT});
  document.querySelectorAll('[data-permission="suggestions"],[data-module="suggestions"]').forEach(btn=>{const x=h(btn);if(!x)return;x.style.backgroundImage='none';x.style.padding='0';x.innerHTML=IDEA});
  document.querySelectorAll('#messaging .m6EmptyIcon,#messaging .m6WelcomeCard span').forEach(x=>{x.style.backgroundImage='none';x.innerHTML=CHAT});
}
const s=document.createElement('style');s.id='unifiedModuleIconsV1Style';s.textContent=`
...
apply();setTimeout(apply,100);setTimeout(apply,500);window.addEventListener('horticulture-users-synced',apply);new MutationObserver(apply).observe(document.documentElement,{subtree:true,childList:true});
})();