...
function loadSortiesSafe(){
  ...
  const loadWizard=()=>{if(document.getElementById('sortiesCreateWizardV1'))return;const w=document.createElement('script');w.id='sortiesCreateWizardV1';w.src='./sorties-create-wizard-v1.js?v=1';w.async=false;document.head.appendChild(w)};
  const loadAttendance=()=>{if(document.getElementById('sortiesAttendanceFastV1')){loadWizard();return;}const f=document.createElement('script');f.id='sortiesAttendanceFastV1';f.src='./sorties-attendance-fast-v1.js?v=1';f.async=false;f.onload=loadWizard;f.onerror=loadWizard;document.head.appendChild(f)};
  const loadDetails=()=>{if(document.getElementById('sortiesDetailsPlusV1')){loadAttendance();return;}const d=document.createElement('script');d.id='sortiesDetailsPlusV1';d.src='./sorties-details-plus-v1.js?v=2';d.async=false;d.onload=loadAttendance;d.onerror=loadAttendance;document.head.appendChild(d)};
