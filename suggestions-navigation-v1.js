...
let restoring=false,suppressRestoreUntil=0;
...
function forceView(id,remember=true){
  if(id==='home')suppressRestoreUntil=Date.now()+1500;
...
document.addEventListener('click',e=>{
  ...
  if(isSuggestionsBack(el)){
    e.preventDefault();e.stopImmediatePropagation();
    localStorage.setItem(PAGEKEY,'home');
    forceView('home',false);
    return;
  }
...
function restore(){css();if(Date.now()<suppressRestoreUntil||restoring||localStorage.getItem(PAGEKEY)!=='suggestionsApp')return;restoring=true;let tries=0;const timer=setInterval(()=>{tries++;if(localStorage.getItem(PAGEKEY)!=='suggestionsApp'||Date.now()<suppressRestoreUntil){clearInterval(timer);restoring=false;return}if(document.getElementById('suggestionsApp')&&window.HorticultureSuggestions?.show){forceView('suggestionsApp',false);clearInterval(timer);setTimeout(()=>{if(localStorage.getItem(PAGEKEY)==='suggestionsApp')forceView('suggestionsApp',false)},350);restoring=false}else if(tries>=30){clearInterval(timer);restoring=false}},100)}
new MutationObserver(()=>{css();if(Date.now()>=suppressRestoreUntil&&localStorage.getItem(PAGEKEY)==='suggestionsApp'&&!suggestionsActive())restore()}).observe(document.documentElement,{childList:true,subtree:true});
