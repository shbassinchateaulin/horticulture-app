...
async function flush(local){
  ...
  try{
    ...
    const fresh=await get('listSortiesAdmin');
    if(fresh&&fresh.ok&&Array.isArray(fresh.sorties)){remoteSnapshot=fresh.sorties;applyingRemote=true;originalSetItem.call(localStorage,STORE,JSON.stringify(fresh.sorties));applyingRemote=false;window.dispatchEvent(new CustomEvent('horticulture-sorties-shared-updated',{detail:fresh}));}
...
window.HorticultureSortiesSharedReady=(async()=>{
  try{
    const j=await get('listSortiesAdmin');
    if(j&&j.ok&&Array.isArray(j.sorties)){
      remoteSnapshot=j.sorties;
      applyingRemote=true;
      originalSetItem.call(localStorage,STORE,JSON.stringify(j.sorties));
      applyingRemote=false;
      window.HorticultureSortiesSharedInfo={connected:true,lastHelloAssoSync:j.lastHelloAssoSync||''};
      return true;
    }
  }catch(e){console.warn('[Sorties] Base commune non encore disponible :',e&&e.message||e)}
  window.HorticultureSortiesSharedInfo={connected:false,lastHelloAssoSync:''};
  return false;
})();
window.HorticultureSortiesShared={refresh:async()=>{const j=await get('listSortiesAdmin');if(j&&j.ok&&Array.isArray(j.sorties)){remoteSnapshot=j.sorties;applyingRemote=true;originalSetItem.call(localStorage,STORE,JSON.stringify(j.sorties));applyingRemote=false;window.dispatchEvent(new CustomEvent('horticulture-sorties-shared-updated',{detail:j}));return j}return j}};
})();
