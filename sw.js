const VERSION='v66-stable-network-6';
...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith((async()=>{
    ...
    ['shared-sync','permanent-delete','access-status-direct','access-live-sync','permission-ui','profile-personalization','account-ui','home-permissions-fix','permissions-cloud-sync','settings-v1','notification-onboarding','profile-v1','profile-restore','account-recovery-v1','access-route-fix','security-alerts-v1','dashboard-v1','dashboard-tune','dashboard-fixes-v2','dashboard-plant-fix','plant-render-fix'].forEach(n=>{text=text.replace(new RegExp('<script src="\\./?'+n+'\\.js[^>]*><\\/script>','g'),'')});
