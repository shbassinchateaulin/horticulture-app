...
const HEADERS = ['id','firstName','lastName','email','function','role','username','passwordHash','firstLogin','active','permissions','updatedAt','phone','photo'];
...
function getSheet_() {
  ...
  if (sh.getLastRow() === 0) {
    ...
    // Migration douce : conserve toutes les lignes existantes et ajoute les nouvelles colonnes.
...
function userRow_(u){return [
  ...
  !!u.firstLogin,u.active!==false,(u.permissions||[]).join(','),new Date().toISOString(),u.phone||'',u.photo||''
]}
