...
function listNotificationsForUser_(userId){const u=notifUser_(userId);if(!u)return{ok:false,error:'Utilisateur introuvable ou inactif'};const sh=notifSheet_(),rows=sh.getLastRow()<2?[]:sh.getDataRange().getValues().slice(1),readSh=notifReadSheet_(),reads=readSh.getLastRow()<2?[]:readSh.getDataRange().getValues().slice(1),readMap={};reads.forEach(r=>{if(String(r[1])===String(userId))readMap[String(r[0])]=String(r[2]||'')});const now=Date.now(),cutoff=now-30*24*60*60*1000,notifications=rows.filter(r=>r[0]).map(r=>({id:String(r[0]),type:String(r[1]||'info'),title:String(r[2]||''),message:String(r[3]||''),targetUsers:String(r[4]||''),targetPermissions:String(r[5]||''),createdAt:String(r[6]||''),expiresAt:String(r[7]||''),data:String(r[8]||'')})).filter(n=>(!n.expiresAt||Date.parse(n.expiresAt)>now)&&(!n.createdAt||!Number.isFinite(Date.parse(n.createdAt))||Date.parse(n.createdAt)>=cutoff)&&notificationVisibleForUser_(n,u)).map(n=>{let data={};try{data=JSON.parse(n.data||'{}')}catch{}return{id:n.id,type:n.type,title:n.title,message:n.message,createdAt:n.createdAt,data:data,read:!!readMap[n.id],readAt:readMap[n.id]||''}}).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));return{ok:true,notifications:notifications}}
...
function markAllNotificationsRead_(userId){const r=listNotificationsForUser_(userId);if(!r.ok)return r;const ids=r.notifications.filter(n=>!n.read).map(n=>String(n.id));if(!ids.length)return{ok:true,marked:0};const sh=notifReadSheet_(),values=sh.getLastRow()<2?[]:sh.getDataRange().getValues(),existing=new Set();for(let i=1;i<values.length;i++)if(String(values[i][1])===String(userId))existing.add(String(values[i][0]));const at=new Date().toISOString(),newRows=ids.filter(id=>!existing.has(id)).map(id=>[id,String(userId),at]);if(newRows.length)sh.getRange(sh.getLastRow()+1,1,newRows.length,3).setValues(newRows);return{ok:true,marked:ids.length}}
