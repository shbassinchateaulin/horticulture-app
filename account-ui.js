...
function me(){const s=session();if(!s)return null;const users=list();const byId=users.find(u=>String(u.id)===String(s.id));if(byId)return byId;const byUsername=users.find(u=>String(u.username||'').toLowerCase()===String(s.username||'').toLowerCase());return byUsername||s}
function get(u,...ks){for(const k of ks){const v=u?.[k];if(v!=null&&String(v).trim())return String(v).trim()}return''}
function superAdmin(u){return String(get(u,'username')).toLowerCase()==='superadmin'||get(u,'id')==='superadmin'}
...
function functionLabel(u,sa){if(sa)return'Super Admin';const fn=get(u,'function','fonction');if(fn)return fn;const role=get(u,'role');return role&&role!=='Compte'?role:'Compte'}
function apply(){const u=me();if(!u)return;const ps=permissions(u),sa=superAdmin(u);
 ...
 const first=get(u,'firstName','firstname','first_name');const last=get(u,'lastName','lastname','last_name');const fn=functionLabel(u,sa);const full=[first,last].filter(Boolean).join(' ');const ini=((first[0]||'')+(last[0]||'')).toUpperCase()||'U';
 setText('#name,#home .name,.welcomeName,.admWelcomeName',first||'Utilisateur');setText('#role,#home .chip,.welcomeRole,.admWelcomeRole',fn);setText('#dname',full||first||'Utilisateur');setText('#drole',fn);
