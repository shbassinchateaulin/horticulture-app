(()=>{
'use strict';
if(window.__horticultureDocumentsSimpleV2)return;window.__horticultureDocumentsSimpleV2=true;
...
let busy=false;
function polish(){if(busy)return;busy=true;try{migrate();const root=document.getElementById('documentsCenter');if(!root)return;
 const intro=root.querySelector('.docCloudIntro p');if(intro&&intro.textContent!=='Documents générés et fichiers de l’association.')intro.textContent='Documents générés et fichiers de l’association.';
 const cards=[...root.querySelectorAll('.docSpaces > button')];for(const b of cards){const t=(b.textContent||'').toLowerCase();if(t.includes('classement')){b.dataset.folder='Fichiers';const strong=b.querySelector('b');if(strong&&strong.textContent!=='Fichiers')strong.textContent='Fichiers';const small=b.querySelector('small');if(small&&small.textContent!=='Tous les dossiers et documents de l’association')small.textContent='Tous les dossiers et documents de l’association';}else if(t.includes('espace collectif')){b.remove();}}
 const spaces=root.querySelector('.docSpaces');if(spaces&&spaces.style.gridTemplateColumns!=='repeat(2, 1fr)')spaces.style.gridTemplateColumns='repeat(2, 1fr)';
 root.querySelector('.docCollectiveNote')?.remove();
 const head=root.querySelector('.docHead h1');if(head&&head.textContent!=='Cloud Documents')head.textContent='Cloud Documents';
 document.querySelectorAll('[data-module="documents-center"] small').forEach(s=>{if(s.innerHTML!=='Documents générés<br>et fichiers')s.innerHTML='Documents générés<br>et fichiers'});
 }finally{busy=false}}
const observer=new MutationObserver(()=>{requestAnimationFrame(polish)});
const app=document.querySelector('#appShell main.app');if(app)observer.observe(app,{childList:true,subtree:true});
