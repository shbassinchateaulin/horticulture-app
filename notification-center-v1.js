...
async function openTray(){open=true;renderTray();try{await window.HorticultureLightBadges?.refreshNow?.()}catch(e){console.warn('Actualisation notifications impossible',e)}if(open)renderTray()}
...
window.HorticultureNotificationCenter={refresh:async()=>{await window.HorticultureLightBadges?.refreshNow?.();return items()},getNotifications:()=>items(),getUnread:()=>items(),markSuggestionRead:id=>window.HorticultureLightBadges?.markSuggestionRead?.(id),open:openTray};
})();