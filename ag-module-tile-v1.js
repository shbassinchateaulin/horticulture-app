...
function cleanup(){
 ...
 document.querySelectorAll('#appShell main.app > .view').forEach(v=>{if(v===ag)return;v.style.removeProperty('display');v.style.removeProperty('visibility');v.style.removeProperty('opacity');v.style.removeProperty('pointer-events')});
}
function showAG(){
 ...
 ag.hidden=false;ag.style.removeProperty('display');ag.style.removeProperty('visibility');ag.style.removeProperty('opacity');ag.style.removeProperty('pointer-events');
 document.getElementById('drawer')?.classList.remove('open');window.scrollTo(0,0);return true;
}
function open(){
 cleanup();const mine=++token;
 const attempt=()=>{if(mine!==token)return true;try{if(typeof window.HorticultureAG?.open==='function'){window.HorticultureAG.open();if(mine===token)showAG();return true}}catch(e){console.error('Consultation AG',e)}return false};
 if(attempt())return;
 [80,220,500,900].forEach(ms=>setTimeout(()=>{if(mine===token)attempt()},ms));
}
function isAGButton(el){return !!el?.closest?.('[data-module="consultation-ag"],[data-permission="consultation_ag"]')}
// Any navigation away cancels every pending delayed AG open BEFORE other handlers run.
window.addEventListener('click',e=>{const t=e.target?.closest?.('[data-go],[data-permission],[data-module],.admBrand,.welcome img,.dhead img');if(!t||isAGButton(t))return;cleanup()},true);
...
wire();setTimeout(wire,250);setTimeout(wire,800);window.addEventListener('horticulture-users-synced',wire);window.HorticultureAGNavigation={open,cleanup};
})();