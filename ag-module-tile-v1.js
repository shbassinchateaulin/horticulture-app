...
function releaseAGWorkspace_(){
  ...
  if(ag){ag.classList.remove('active');ag.hidden=false}
}
...
function showHome_(){pendingSaveOpen='';releaseAGWorkspace_();const shell=document.getElementById('appShell');if(shell){shell.style.setProperty('display','block','important');shell.style.setProperty('visibility','visible','important');shell.style.setProperty('opacity','1','important')}const main=document.querySelector('main.app');if(main){main.style.setProperty('display','block','important');main.style.setProperty('visibility','visible','important');main.style.setProperty('opacity','1','important')}document.querySelectorAll('main.app > .view').forEach(v=>v.classList.remove('active'));const home=document.getElementById('home');if(home){home.hidden=false;home.classList.add('active')}window.scrollTo(0,0)}
function repairAfterBack_(){setTimeout(()=>{const ag=document.getElementById('agConsultation');if(document.body.classList.contains('agWorkspaceMode')&&ag?.classList.contains('active'))forceAGVisible_();else showHome_()},0)}
function loadTransmission_(){if(document.getElementById('agDistributionStableDirect'))return;document.getElementById('agTransmissionDirect')?.remove();document.getElementById('agTransmissionSafe')?.remove();const s=document.createElement('script');s.id='agDistributionStableDirect';s.src='./ag-distribution-stable.js?v=10&t='+Date.now();s.async=false;document.head.appendChild(s)}
function restoreAGAfterRefresh_(n=0){const r=readAGRoute_();if(!isAGActive_()||!r)return false;const api=window.HorticultureAG;if(!api){if(n<30)setTimeout(()=>restoreAGAfterRefresh_(n+1),80);return false}try{if(r.screen==='campaign'&&r.id&&typeof api.openCampaign==='function'){const tab=['overview','collect','responses','results','settings'].includes(r.tab)?r.tab:'overview';api.openCampaign(r.id,tab);forceAGVisible_();loadTransmission_();return true}api.open?.();forceAGVisible_();return true}catch(e){console.error('Restauration Consultation AG',e)}return false}
function runAG_(){document.getElementById('drawer')?.classList.remove('open');releaseAGWorkspace_();try{if(typeof window.HorticultureAG?.open==='function'){window.HorticultureAG.open();forceAGVisible_();loadTransmission_();setTimeout(forceAGVisible_,60);return true}}catch(e){console.error('Ouverture Consultation AG',e)}return false}
...
function releaseBeforeOtherNavigation_(e){const t=e.target?.closest?.('[data-permission],[data-module],[data-go],.admBrand,.welcome img,.dhead img');if(!t)return;if(t.matches?.('[data-module="consultation-ag"],[data-permission="consultation_ag"]'))return;if(document.body.classList.contains('agWorkspaceMode')||isAGActive_())releaseAGWorkspace_()}
...
document.addEventListener('click',e=>{if(e.target.closest?.('#agConsultation [data-ag-back],#agConsultation .agPageBack,#agConsultation .back'))repairAfterBack_()},false);
document.addEventListener('click',e=>{const home=e.target.closest?.('[data-go="home"],.admBrand,.admBrand img,.top b,.welcome img,.dhead img');if(!home)return;setTimeout(showHome_,0)},false);
window.addEventListener('click',releaseBeforeOtherNavigation_,true);
