...
function loadDocumentsCenter(){if(document.getElementById('documentsCenterV2'))return;const s=document.createElement('script');s.id='documentsCenterV2';s.src='./documents-center-v1.js?v=2';s.async=false;s.onload=loadDocumentsSimple;document.head.appendChild(s)}
function loadDocumentsSimple(){if(document.getElementById('documentsCenterSimpleV1'))return;const s=document.createElement('script');s.id='documentsCenterSimpleV1';s.src='./documents-center-simple-v1.js?v=1';s.async=false;document.head.appendChild(s)}
loadDocumentsCenter();
...
function open(){
 ...
 try{window.HorticultureAG?.open?.()}catch(e){console.error('Consultation AG',e)}
 const ag=prepareAG();if(!ag)return;
 document.querySelectorAll('#appShell main.app > .view').forEach(v=>{v.classList.toggle('active',v===ag);v.style.removeProperty('display');v.style.removeProperty('visibility');v.style.removeProperty('opacity');v.style.removeProperty('pointer-events')});
 document.getElementById('drawer')?.classList.remove('open');window.scrollTo(0,0)
}
...
wire();setTimeout(wire,250);setTimeout(wire,800);window.addEventListener('horticulture-users-synced',wire);window.HorticultureAGNavigation={open};
})();