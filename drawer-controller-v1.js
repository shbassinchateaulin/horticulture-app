...
function openAdherents(){clearCurrent();close();const fn=window.HorticultureAdherents?.open;if(typeof fn==='function'){try{fn()}catch(e){console.error('Adhérents',e)}}// The legacy module may use its own stale view list after Home. Reassert the actual dynamic view after it has run.
requestAnimationFrame(()=>{forceAdherents();setTimeout(forceAdherents,30)});return !!fn||!!document.getElementById('adherentsAdmin')}
...
function route(btn){const label=norm(btn.textContent),go=btn.dataset?.go;if(label.includes('accueil')||go==='home')return home();if(label.includes('adherent'))return openAdherents();if(go&&document.getElementById(go))return activate(go);const target=findHomeTarget(btn);if(target){clearCurrent();close();setTimeout(()=>target.click(),0);return true}neutralizeAG();close();return false}
document.addEventListener('click',e=>{const menu=e.target.closest?.('#menu,.menuBtn');if(menu&&menu.closest('#appShell')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();toggle();return}const d=drawer();if(!d)return;if(e.target===d){e.preventDefault();e.stopImmediatePropagation();close();return}const btn=e.target.closest?.('.dlist button');if(btn&&d.contains(btn)){const handled=route(btn);if(handled){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}}},true);
...
window.HorticultureDrawer={open,close,toggle,home,activate,openAdherents,forceAdherents,neutralizeAG};
})();