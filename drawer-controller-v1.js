...
function home(){const nav=window.HorticultureMessagingNavigation?.showHome;if(typeof nav==='function')return nav();try{sessionStorage.removeItem('horticulture-messaging-route-v1')}catch(_){}document.body.classList.remove('m6MessagingActive','m6KeyboardOpen','messaging-open');return activate('home')}
function forceAdherents(){const v=document.getElementById('adherentsAdmin');if(!v)return false;neutralizeAG();views().forEach(x=>x.classList.toggle('active',x===v));v.hidden=false;v.style.removeProperty('display');v.style.removeProperty('visibility');v.style.removeProperty('opacity');v.style.removeProperty('pointer-events');close();window.scrollTo(0,0);return true}
function invokeAdherents(){const fn=window.HorticultureAdherents?.open;if(typeof fn!=='function')return false;try{fn()}catch(e){console.error('Adhérents',e);return false}return true}
...
function ensureMessagingEntry(){
  ...
  btn=document.createElement('button');btn.type='button';btn.dataset.module='messaging';btn.textContent='Messagerie';
...
function route(btn){const label=norm(btn.textContent),go=btn.dataset?.go,permission=String(btn.dataset?.permission||'').toLowerCase(),module=String(btn.dataset?.module||'').toLowerCase();if(btn.id==='logout'||label.includes('deconnexion'))return logout();if(label.includes('accueil')||go==='home')return home();if(label.includes('publier')||label.includes('actualite')||permission==='communication'||module==='publish'||go==='publish')return activate('publish');if(label.includes('messagerie')||permission==='messaging'||module==='messaging')return openMessaging();if(label.includes('adherent')||permission==='adherents'||module==='adherents')return openAdherents();if(go&&document.getElementById(go))return activate(go);const target=findHomeTarget(btn);if(target){clearCurrent();close();setTimeout(()=>target.click(),0);return true}neutralizeAG();close();return false}
document.addEventListener('click',e=>{const menu=e.target.closest?.('#menu,.menuBtn');if(menu&&menu.closest('#appShell')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();toggle();return}const pub=e.target.closest?.('#home [data-permission="communication"],#home [data-module="publish"],#home [data-go="publish"]');if(pub&&pub.closest('#appShell')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();activate('publish');return}const adh=e.target.closest?.('[data-permission="adherents"],[data-module="adherents"]');if(adh&&adh.closest('#appShell')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openAdherents();return}const d=drawer();if(!d)return;if(e.target===d){e.preventDefault();e.stopImmediatePropagation();close();return}const btn=e.target.closest?.('.dlist button');if(btn&&d.contains(btn)){const handled=route(btn);if(handled){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation()}}},true);
